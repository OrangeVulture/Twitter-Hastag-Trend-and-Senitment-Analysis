...
if __name__ == "__main__":
	...
	# sum_val_table = tweets_table.select(sum('senti_val').alias('avg_senti_val'))

	expr = [last(col).alias(col) for col in tweets_table.columns]

	wow = tweets_table.agg(*expr)
	...
	query2 = wow.writeStream.outputMode("complete").format("console").start()
